...
export default function GitHubDashboard() {
  ...
  const loadRepos = async () => { setLoading(true); setError(''); try { const data = await github.listRepos(); setRepos(data); setAdsRepo(current => current || data.find(repo => repo.name.toLowerCase().includes('ads'))?.full_name || ''); } catch (err: any) { setError(err.message || 'Unable to load GitHub repositories.'); } finally { setLoading(false); } };
  useEffect(() => { loadRepos(); }, []);
  ...
  return <div className="space-y-6">
    ...
    {tab === 'repos' && (loading ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">{[1, 2, 3].map(item => <div key={item} className="h-40 rounded-xl bg-[#0b0f19] border border-[#1a2234] animate-pulse" />)}</div> : <div className="space-y-4"><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">{repos.map(repo => <RepoCard key={repo.id} repo={repo} onSelect={selected => setSelectedRepo(selected.full_name)} />)}{repos.length === 0 && !error && <p className="text-xs text-slate-500">No repositories are available.</p>}</div>{selectedRepo && <GitHubExplorer repo={selectedRepo} onFileSelect={selectFile(selectedRepo)} />}</div>)}
    ...
    {tab === 'ads' && <div className="space-y-4"><select value={adsRepo} onChange={event => setAdsRepo(event.target.value)} className="w-full sm:w-80 px-3 py-2.5 bg-[#0b0f19] border border-[#1a2234] rounded-lg text-xs font-mono text-slate-200 focus:outline-none focus:border-cyan-500" disabled={loading || adsRepos.length === 0}><option value="">{loading ? 'Loading repositories…' : 'No Google Ads repositories found'}</option>{adsRepos.map(repo => <option key={repo.id} value={repo.full_name}>{repo.name}</option>)}</select>{adsRepo && <GitHubExplorer repo={adsRepo} onFileSelect={selectFile(adsRepo)} />}</div>}

...
interface RepoCardProps { key?: React.Key; repo: GitHubRepo; onSelect?: (repo: GitHubRepo) => void; className?: string; }
...
export default function RepoCard({ repo, onSelect, className = '' }: RepoCardProps) {
  return <article className={`bg-[#0b0f19] border border-[#1a2234] rounded-xl p-4 shadow-lg hover:border-cyan-500/40 transition-colors ${className}`}>
    ...
    <footer className="mt-4 pt-3 border-t border-[#1a2234] flex items-center justify-between gap-2 text-[10px] font-mono text-slate-500"><span className="flex items-center gap-1 truncate"><GitBranch size={12} /> {repo.language || 'Unknown'}</span><span>{relativeTime(repo.updated_at)}</span></footer>
  </article>;
}
